...
## prose.io
完全地线上写博客，不用git pull/commit/push，那就要借道<prose.io>，只要你的github账号授权给他，他就可以替你完成git的一些操作，让你专心写作，我的这边博文就是在prose.io写的  

而且在prose.io写还有其他的好处，比如新建一篇文章的时候，默认就带上了当前日期作为标题，就省去自己又去看日期然后把日期写上去，还有它有在线预览功能，如果在_config.yml配置了siteurl，那么预览功能就可以直接按照siteurl样式生成页面预览，而不用用默认的预览页面，这样更加贴近发布后的显示状况  
...
```
我在_config.yml里面关于prose.io的设置，rooturl是让你打开prose.io里面你的pages项目的时候，能够直接进去_posts/这个目录，相当于省了一次点击吧，然后siteurl上面也说了，我就不重复了
