...
- 3 `WHERE name IN (SELECT name FROM grade WHERE score<60 GROUP BY name HAVING COUNT(score)>=2)`
    ...
    - 3-4 `GROUP BY name`，相同姓名归一组。**GROUP BY简单来说是建立组**  
    注明:截图中sn为name,cn为class,sc为score,下同  
    ...
    - 3-7 再搞括号外的
    - 所以`WHERE name IN ( SELECT name FROM grade WHERE score<60 GROUP BY name HAVING COUNT(score)>=2)`就变成了`WHERE name IN (吴克)`，故name的取值只能是`吴克`，最后得到:
