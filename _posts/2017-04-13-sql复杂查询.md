...
- 3. `WHERE name IN ( SELECT name FROM grade WHERE score<60 GROUP BY name HAVING COUNT(score)>=2)`
    - 括号内`SELECT name FROM grade WHERE score<60 GROUP BY name HAVING COUNT(score)>=2`
    - `FROM score`,我们可以得到原来的表如下  
    ...
    - `WHERE score<60`，我们可以筛出得分小于60的项的表，如下  
    ...
    - `GROUP BY name`，相同姓名归一组。**GROUP BY简单来说是建立组**，关于GROUP BY[2]会讲的更加好理解  
    ...
    - `HAVING COUNT(score)>=2`，HAVING的作用和WHERE很像都是筛选，不过WHERE不能带聚合函数的(比如SVG,SUM,COUNT等等)，**HAVING是用来筛选组的**，所以往往和GROUP BY配合出现  
    ...
    - `SELECT name`，根据上面HAVING筛选结果自然就只有`吴克`了  
