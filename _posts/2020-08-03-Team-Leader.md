...
提高review的速度:你并没有处于需要专注工作的时候，那么你应该在CL被提交后尽快进行review，review回复最长的极限是一个工作日，但是开发的时候，打断自己实际上会比让另一位开发者等待review来得更加昂贵 => 但是随着代码库的质量越来越高，后面Review速度会越来越快
...
* Review粒度(时间越充分越往下面的阶段进行，但至少覆盖第一和第二阶段)
  * 第一阶段：代码是否规范（lint、命名、注释）
  * 第二阶段：目录结构是否合理
  * 第三阶段：文件、函数是否过长
