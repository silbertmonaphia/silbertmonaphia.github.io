...
为什么我讲那么久原理呢？因为mosh和ssh在作用上实在太像了，而什么安装指南和使用的tutorial，我觉得[Mosh官网](https://mosh.org)就已经写的很好了，如果还要说什么的话，那就是我的话，是用brew install mosh和sudo apt-get install mosh装的本地(OSX)和远程主机(Ubuntu)的mosh，以及我只是简单在远程主机上输入`mosh-server`然后回车，最后别把原来ssh22端口关了就好了，仅此而已。
