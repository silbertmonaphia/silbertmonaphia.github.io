...
我所知道的关于svn实际上常用的实践是，trunk分支(也就是svn的主分支，相当于git的master分支)对应本地的一个目录，然后在线上创建一个trunk的副本——demo测试分支，然后checkout拉取这个demo到本地，本地提交都可以直接和demo打交道，而不用污染trunk主分支。当你觉得是时候发布的时候，就把demo对应的本地目录里面的文件复制到trunk对应的本地目录，然后直接commit上去，这可以是一次相当大量修改的commit。  
